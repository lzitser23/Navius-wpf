...
Policy (2026-07-11, decided while splitting Calendar and DatePicker out of date-range-picker.md; see issue #2): **every public registry item (`registry/registry.json`) gets its own parity document here and its own generated docs page. Internal infrastructure may remain grouped** into the parity doc of the family that owns it (currently: `core`, `anchored-popup` documented across the popup families, `overlay-surface` in dialog/alert-dialog/drawer, `button-automation-peer` in button). Calendar and DatePicker are independent public registry items with distinct APIs, so folding them into the composite's doc violated this rule; calendar.md and date-picker.md fix that. The `registry:styled` items are not web primitive families and are outside this directory's web-parity scope.

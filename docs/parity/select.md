...
Because this is not ComboBox-derived, the free `ComboBoxAutomationPeer`/`ComboBoxItemAutomationPeer` (with `IExpandCollapseProvider`/`ISelectionProvider`/`ISelectionItemProvider`) are unavailable. A minimal `NaviusSelectAutomationPeer` reports `AutomationControlType.ComboBox` for the root and `NaviusSelectItemAutomationPeer` reports `AutomationControlType.ListItem` for items (with the item's label as its name). Full UIA pattern providers (ExpandCollapse / Selection / SelectionItem) are **not** implemented; an open gap, noted here rather than burning time on a full pattern surface for this wave.

...
Shipped as `Controls/Fieldset/`: `NaviusFieldset` + `NaviusFieldsetLegend`. Tier A in mechanism rather than in base class: the core contract behavior ("fieldset disabled disables every contained control for free") maps directly onto WPF's own IsEnabled tree inheritance, so `Disabled` simply drives `IsEnabled` and every descendant control (including `NaviusField`s, whose effective disabled state is exactly the OR of their own flag and any ancestor's, matching the web's `Disabled || Fieldset.Disabled`) disables without any reimplemented propagation. `GroupBox` itself was NOT used: its ControlTemplate pins the header into the border, whereas the contract's Legend is deliberately free-floating "for positioning freedom", so the root is a plain lookless ContentControl and `NaviusFieldsetLegend` is an ordinary child placeable anywhere, which is the same freedom the web `<div>` legend has.

Deltas, per the open questions:

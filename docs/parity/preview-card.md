...
Shipped as `Navius.Wpf.Primitives.Controls.NaviusPreviewCard` (`src/Navius.Wpf.Primitives/Controls/PreviewCard/NaviusPreviewCard.cs`), a lookless `ContentControl` built on `NaviusAnchoredPopup`, non-modal end to end matching the web's `MoveFocusInside = false`/`Modal` hardcoded `false`.

- **Part collapse.** Trigger and Positioner collapse onto the root control: `Content` is the Trigger (rendered inside a focusable `Border` template part, not a true `Hyperlink`/`Button` control, per the "Trigger... should map to a focusable Button/HyperlinkButton-style WPF element" strategy note; a `Border` was chosen for simplicity and still supports Tab stops and `GotKeyboardFocus`/`LostKeyboardFocus`), `PreviewContent`/`Side`/`Align`/`SideOffset`/`AlignOffset` are the Positioner. There is no separate `NaviusPreviewCardBackdrop`; the contract itself notes it is "rarely used since preview cards are non-modal."
- **`Modal` dropped** (resolves the first open question above): `NaviusPreviewCard` has no `Modal` property at all, rather than keeping a dead always-`false` one, since `OverlayOptions.Modal` already defaults to `false` and nothing in the WPF port reads a PreviewCard-specific `Modal` flag.
- **Hover-region bridge** (resolves the third open question above): the popup content (`PART_PopupContent`) and the trigger share one hover-intent state machine on the control instance itself (a single `DispatcherTimer` pair, `_openTimer`/`_closeTimer`), rather than each side owning independent state. `MouseEnter` on either resets the pending close timer; `MouseLeave` on either (re)starts it, exactly mirroring the web's `@onpointerenter="Context.OpenNow"` / `@onpointerleave="Context.CloseDelayed"` on both Trigger and Popup.
- **Escape-closes-immediately confirmed** (resolves the second open question above): `OverlayOptions.CloseOnEscape = true` is set explicitly when opening, and `OverlayOptions` has no separate close-delay concept, so Escape always closes immediately regardless of `CloseDelay`, matching the inferred web behavior.
- **`RegisterInputRoot` usage.** The popup content is registered as an `OverlaySession` input root, and `CloseOnOutsideClick` is set `true` (not explicitly specified by the web contract, but a reasonable default for a non-modal floating overlay in the WPF port; documented here as a judgment call rather than an inferred parity fact).

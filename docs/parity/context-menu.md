...
Implemented in `src/Navius.Wpf.Primitives/Controls/ContextMenu/` (namespace `Navius.Wpf.Primitives.Controls.ContextMenu`, no collision issue - unlike the Menu family's folder, see menu.md) and `src/Navius.Wpf.Primitives/Themes/ContextMenu.xaml`. Per the task brief's own framing ("same item stack"), this family is intentionally small: it reuses the Menu family's `NaviusMenuItem`/`NaviusMenuCheckboxItem`/`NaviusMenuRadioItem`/`NaviusMenuGroupLabel` (and the general-purpose `NaviusSeparator`) verbatim rather than duplicating near-identical classes, since both parity docs' part/parameter tables are line-for-line the same at the item level. `Themes/ContextMenu.xaml` merges `Themes/Menu.xaml` so a page only has to merge one dictionary to get both the ContextMenu-specific chrome and the shared item styles.

Only two new classes were needed:

- **`NaviusContextMenuTrigger : ContentControl`** - the contract's `<div>` surface with no ARIA. Its `Menu` DP assigns the popup straight onto `FrameworkElement.ContextMenu`, so native `ContextMenuService` handles right-click, Shift+F10, and the keyboard Apps/Menu key opening for free - this resolves this doc's own open question in favor of the native entry point over a custom `RequestOpenAtAsync`-style API. `Disabled` maps onto `ContextMenuService.SetIsEnabled(this, !value)` (the native attached property built for exactly this "suppress the menu, keep the element interactive" case), not `IsEnabled`.
- **`NaviusContextMenuPopup : ContextMenu`** - same class shape as `NaviusMenuPopup` (Side/Align DPs), but the constructor leaves `Placement` untouched: `ContextMenuService`'s internal open flow (both right-click and the keyboard path) sets `Placement = PlacementMode.MousePoint` itself before opening, which already matches the contract's cursor-anchored popup for free. `Side`/`Align` only matter for the explicit `RequestOpenAt(UIElement target, Point pointRelativeToTarget)` method - the contract's `Open(point)` entry point - which switches to `PlacementMode.Custom` for that one open and reuses `NaviusMenuPopup`'s `PlacementMath`-based offset computation, anchored at a zero-size rect at the given point instead of a target element's bounds. Defaults to Side=Right/Align=Start, matching the contract's `NaviusContextMenuPositioner` defaults.

**Part collapse**, beyond the shared item stack already covered in menu.md: `NaviusContextMenuPortal` is a no-op (same reasoning as the Menu family); `NaviusContextMenuPositioner` folds into `NaviusContextMenuPopup`'s own Side/Align properties; `NaviusContextMenuArrow` is not implemented (deferred); `NaviusContextMenuSub`/`SubTrigger`/`SubContent` collapse into native `MenuItem` nesting, identical to the Menu family.

**Answered open questions:**
- *Part decomposition*: collapsed to two classes (Trigger, Popup) plus the reused item stack, not a fine-grained Portal/Positioner/Arrow breakdown - `Popup` already owns placement.
- *Tri-state CheckboxItem*: reuses the Menu family's `Checked`/`IsIndeterminate` solution unchanged.
- *Long-press / Shift+F10*: `ContextMenuService` already handles Shift+F10 and the Apps/Menu key with trivial native rect measurement; an explicit `RequestOpenAt(target, point)` method was kept anyway (not `async`, since no interop round-trip is needed) as the direct mapping of the contract's `Open(point)` API, for callers that need a custom gesture (e.g. long-press) or a non-cursor anchor point. Touch long-press itself is **not implemented**: `ContextMenuService` has no built-in long-press gesture; a custom `TouchDown`/`Stylus` timer would be needed and was out of scope here.
- *TextSearch*: resolved in the Menu family's `NaviusMenuItemBase.TextValue` -> `TextSearch.Text`, reused as-is.

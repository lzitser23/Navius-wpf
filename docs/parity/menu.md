...
**Tests:** `tests/Navius.Wpf.Tests/MenuTests.cs` (17 facts, plus ContextMenuTests's 6 = 23 total, all `[StaFact]`). Covers template application, trigger open/close/resync-on-external-close, Select raise + PreventDefault + close/keep-open, submenu-header click deferring to base (does not close), Command execution, `TextValue` -> `TextSearch.Text`, checkbox toggle/indeterminate/CheckedChanged, radio single-selection (same group, cross-group independence, re-click no-op), `IsEnabled` cascade, and the `MenuItemAutomationPeer` `IToggleProvider`/`ToggleState` mapping.

## M6 audit (2026-07-09)

Adversarial parity re-audit of the Menu family against the C#/XAML. Every keyboard-table row that the "WPF implementation notes" delegate to native `MenuItem`/`ContextMenu` (roving Arrow keys, Home/End, typeahead-via-`TextSearch.Text`, submenu Arrow open/close, Escape) was traced to a real native substrate or a wired override; no false keyboard claim was found in the notes (the JS references in the "Keyboard" table describe the original web contract, not the port). `Themes/Menu.xaml` uses `DynamicResource` exclusively and every key resolves in `Tokens.Light.xaml`/`Tokens.Dark.xaml`. `NaviusMenuItemBase.TextValue` writes `TextSearch.Text` (verified `OnTextValueChanged`); `NaviusMenuCheckboxItem` toggle/indeterminate resolution and `NaviusMenuRadioItem` group enforcement match the notes; `MenuItemAutomationPeer`'s `IToggleProvider` is exercised by test.

### CONFIRMED (fixed)

- **Doc test count wrong.** "WPF implementation notes" claimed `MenuTests.cs` has "23 facts ... = 29 total". Actual: `MenuTests.cs` has **17** `[StaFact]` methods; `ContextMenuTests.cs` has 6, so the real combined total is **23** (the doc's "23" was the total, mislabeled as the `MenuTests` count). Doc corrected to "17 facts, plus ContextMenuTests's 6 = 23 total".

### PLAUSIBLE (not fixed)

- `NaviusMenuCheckboxItem.CheckedProperty` registers a default of `false`, whereas the contract's `Checked`/`DefaultChecked` default is `null` (indeterminate) and the sibling `NaviusMenubarCheckboxItem` defaults to `null`. A default-constructed Menu checkbox therefore starts unchecked instead of indeterminate, and (because the indeterminate glyph is driven by the separate `IsIndeterminate` DP, itself defaulting `false`) would not show the dash glyph until `Checked` is explicitly set. Low impact per the doc's own open question (indeterminate is only reachable programmatically), and fixing it correctly would also require re-driving the glyph, so left as a reported inconsistency rather than a surgical fix.

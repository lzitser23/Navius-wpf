...
**Space adversarial check: Space is NOT dead, CONFIRMED wired and tested.** `HandleKey` handles `case Key.Space` (`NaviusTree.cs:257-267`): Shift+Space in multi-select calls `SelectContiguous`, otherwise `SelectFocused`, which routes through `TreeSelectionState.ReplaceSelection`/`ToggleSelection` -> `ApplySelection` and mutates `NaviusTreeNode.IsSelected` + `SelectedValues` (`NaviusTree.cs:390-401,491-516`). The single keyboard entry point is a real routed hook: `OnPreviewKeyDown` overrides the framework virtual and delegates to `HandleKey`, marking navigation keys handled (`NaviusTree.cs:160-169`). Tests assert real selection change: `HandleKey_Space_SingleMode_ReplacesSelection` (`TreeTests.cs:360`), `HandleKey_Space_MultiMode_Toggles` (`TreeTests.cs:547`), `HandleKey_NoneMode_SpaceDoesNotSelect` (`TreeTests.cs:560`).

**Verified TRUE (with file:line):**

- Arrow expand/collapse/navigate with ltr/rtl mirroring: `expandKey`/`collapseKey` resolved from `FlowDirection` (`NaviusTree.cs:198-200`), `ExpandOrIntoChild`/`CollapseOrToParent` (`NaviusTree.cs:423-456`). Tests `HandleKey_ArrowRight_Ltr_ExpandsCollapsedNode`, `HandleKey_ArrowLeft_Ltr_CollapsesExpandedNode`, `HandleKey_ArrowKeys_AreMirroredUnderRtl` (`TreeTests.cs:373,386,398`).
- Home/End: `NaviusTree.cs:228-250`, tests `TreeTests.cs:337,348`; no-wrap on Up/Down confirmed by `NextEnabledIndex` returning -1 at edges (`TreeSelectionState.cs:34-50`, test `HandleKey_Down_DoesNotWrapPastLastVisible`, `TreeTests.cs:325`).
- Typeahead with 500ms reset: `TypeaheadResetMs = 500` (`NaviusTree.cs:24`), buffer reset logic (`NaviusTree.cs:466-483`), same-char cycling + disabled-skip in `TreeSelectionState.Typeahead` (`TreeSelectionState.cs:117-149`). Tests `TreeTests.cs:221,232,245,451`.
- Multi-select with Ctrl/Shift: Ctrl+A toggle-all, Shift+Arrow extend, Ctrl+Shift+Home/End range-to-edge, Shift+Space contiguous span, Ctrl+Click/Shift+Click mouse (`NaviusTree.cs:99-116,205-280,333-346,403-421`). Tests `TreeTests.cs:465-625`.
- `*` expand-siblings via both `Key.Multiply` and Shift+8 (`NaviusTree.cs:269-275`), test `HandleKey_Asterisk_ExpandsAllSiblingsAtLevel` (`TreeTests.cs:412`).
- Enter activates (toggle expansion then apply selection): `NaviusTree.cs:252-255` -> `Activate` (`NaviusTree.cs:367-388`), test `HandleKey_Enter_TogglesExpansionThenSelectsInSingleMode` (`TreeTests.cs:428`).

**AutomationPeer gap: the doc's description is ACCURATE and precise (verified, NOT fixed per scope).** The claim that individual items' `SelectionItemPattern.IsSelected` is not wired holds: `NaviusTreeItem` deliberately does not override `OnCreateAutomationPeer` (`NaviusTreeItem.cs:17-55`), so it gets the native `TreeViewItemAutomationPeer` whose `ISelectionItemProvider.IsSelected` reads native `TreeViewItem.IsSelected`, which this port never sets (selection lives on `NaviusTreeNode.IsSelected`, `NaviusTreeNode.cs:83-96`). Test `ItemAutomationPeer_UsesNativeTreeViewItemPeer` (`TreeTests.cs:723`) confirms the native peer. The root-level `NaviusTreeAutomationPeer` does correctly re-implement `ISelectionProvider.CanSelectMultiple`/`IsSelectionRequired` off `SelectionMode` and a best-effort `GetSelection()` limited to realized containers (`NaviusTreeAutomationPeer.cs:24-40`, `NaviusTree.TryGetRealizedPeer` at `NaviusTree.cs:128-147`). Left untouched: the per-item `SelectionItemPattern` wiring is owned by the separate high-contrast/UIA agent.

> **Update (same day, high-contrast/UIA agent):** this gap is now fixed. `NaviusTreeItem.OnCreateAutomationPeer` returns a new `NaviusTreeItemAutomationPeer`; the old `ItemAutomationPeer_UsesNativeTreeViewItemPeer` test (which asserted the gap) was replaced by
> `ItemAutomationPeer_IsCustomPeer_StillDerivesTreeViewItemPeer` plus several new provider-logic tests. See the "WPF implementation notes" section above (now marked "fixed, M6") and ADR-0007 for the full change.

**Confirmed issues fixed:** none in this batch's scope (the one known UIA delta is out of scope and left to the UIA agent).

**Plausible/residual:** roving-tabindex is an approximation and `FocusMode="activedescendant"` was dropped, both already documented above as deliberate simplifications. (Unrelated: the shared test assembly did not compile at audit time due to a concurrent Select-family edit to `SelectTests.cs`; outside scope, untouched.)

...
The WPF control also exposes token-backed `Variant` (`Default`, `Secondary`, `Outline`, `Ghost`, `Destructive`) and `Size` (`Default`, `Small`, `Large`, `Icon`) dependency properties. These are WPF styled-layer conveniences rather than web primitive parity behavior; they keep consumers on the shipped template instead of requiring app-owned replacement styles for common button treatments.
